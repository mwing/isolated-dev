// Command dev-proxy is the egress sidecar. It runs inside the container
// network as the only host with a route out, enforcing the allowlist for
// both connections and name resolution.
//
// It is deliberately a separate binary from dev: it runs in a different
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, "dev-proxy:", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	// container: `dev-proxy control allow example.com`. It exists here
	...
	rules := allow.Rules()
	if len(rules) == 0 {
		fmt.Fprintln(os.Stderr, "dev-proxy: empty allowlist; all egress will be denied")
	} else {
		names := make([]string, 0, len(rules))
		for _, r := range rules {
			names = append(names, r.String())
		}
		fmt.Fprintf(os.Stderr, "dev-proxy: allowing %s\n", strings.Join(names, " "))
	}
	...
	if *askTimeout > 0 {
		fmt.Fprintf(os.Stderr, "dev-proxy: holding denied connections up to %s for a decision\n",
			*askTimeout)
	}
	...
	if !*noDNS {
		...
		fmt.Fprintf(os.Stderr, "dev-proxy: resolver listening on %s\n", *dnsAddr)
	}
	...
	if *ctlPath != "" {
		...
		ctl.OnChange = func(op, host string, rules []string) {
			...
			fmt.Fprintf(os.Stderr, "dev-proxy: %s %s; policy now: %s\n",
		...
		fmt.Fprintf(os.Stderr, "dev-proxy: control socket at %s\n", *ctlPath)
	}
	...
	for _, line := range netpolicy.Summary(proxy.Denials()) {
		fmt.Fprintln(os.Stderr, "dev-proxy:", line)
	}
	return nil
}
...
func control(path string, args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: dev-proxy control <allow|revoke|list|denials> [host]")
	}
...
const readyLine = "dev-proxy: ready"
