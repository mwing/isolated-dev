...
type Options struct {
	...
	Ports          []int
	// EgressFiltered reports whether dev2 runs this project behind the
	// egress proxy, which an IDE will not do.
	EgressFiltered bool
	// Tools were added with `dev2 tools add`; they are baked into the
...
func Generate(o Options) Generated {
	...
	b.WriteString("{\n")
	b.WriteString("  // Generated by `dev2 devcontainer`. Edit or regenerate.\n")
	b.WriteString("  //\n")
	b.WriteString("  // This describes the same image dev2 builds, so an editor's\n")
	b.WriteString("  // dev container gives the same tools and the same non-root user.\n")
	b.WriteString("  // It does NOT reproduce dev2's egress filtering: that lives in a\n")
	b.WriteString("  // proxy sidecar dev2 starts, and an editor will not start it.\n")
	...
	// The same unprivileged uid dev2 runs as, so files written in the
	// editor belong to the same user as files written by dev2.
	...
	// runArgs mirror the hardening dev2 applies. An editor that ignores
	...
	if o.EgressFiltered {
		g.Notes = append(g.Notes,
			"egress filtering is not reproduced: an editor runs the container with "+
				"normal network access, while dev2 routes it through an allowlisting proxy")
	}
	if len(o.Tools) > 0 {
		g.Notes = append(g.Notes,
			"tools added with `dev2 tools add` are baked into the generated Dockerfile: "+
				strings.Join(o.Tools, ", "))
	}
	g.Notes = append(g.Notes,
		"trust decisions are dev2's own: an editor will not ask before honoring "+
