...
func listTools(ctx context.Context, env *Env) error {
	...
	if len(tools) == 0 {
		...
		fmt.Fprintf(env.Stdout, "  dev2 tools add <tool>      add it, permanently\n")
		return nil
	}
...
func searchTools(ctx context.Context, env *Env, term string, limit int) error {
	...
	if exists, err := eng.ImageExists(ctx, image); err != nil {
		...
		return fmt.Errorf("the project image is not built yet; run `dev2 build` first")
	}
	...
	spec := container.RunSpec{
		...
		Labels:  map[string]string{"dev2.role": "tool-search"},
	}
	...
	fmt.Fprintf(env.Stdout, "\nAdd one with: dev2 tools add <name>\n")
	return nil
}
