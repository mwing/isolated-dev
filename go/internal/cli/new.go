...
import (
	...
	"github.com/mwing/isolated-dev/go/internal/scaffold"
)
...
func newNewCmd(env *Env) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Creates the plugin's scaffolding files. It does not write a\n" +
			...
			"stale. Write one when you want to change it — `dev2 build` uses a\n" +
...
func newProject(env *Env, language, dir, version string, force bool) error {
	...
	fmt.Fprintf(env.Stdout, "  dev2 run -c '<command>'   build and run, sandboxed\n")
	fmt.Fprintf(env.Stdout, "  dev2 pin                  fix the base image to a digest\n")
	return nil
}
