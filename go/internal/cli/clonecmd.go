...
func listClones(ctx context.Context, env *Env) error {
	...
	if len(clones) == 0 {
		fmt.Fprintln(env.Stdout, "No clones. `dev2 run --clone` makes one.")
		return nil
	}
	...
	if holding > 0 {
		fmt.Fprintf(env.Stdout, "%d hold work that is not in their project. "+
			"`dev2 clone rm` refuses those until it is fetched back.\n", holding)
	}
	return nil
}
...
func newClonePathCmd(env *Env) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			if _, err := os.Stat(filepath.Join(path, ".git")); err != nil {
				return fmt.Errorf("this project has no clone yet; `dev2 run --clone` makes one")
			}
...
func newCloneRemoveCmd(env *Env) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Deletes this project's clone, or a named one from `dev2 clone list`.\n\n" +
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if (dirty > 0 || unmerged > 0) && !force {
				return fmt.Errorf("%s still holds work: %d commit(s) that exist only "+
					...
					"  Or discard it:  dev2 clone rm --force",
