...
func newCompletionInstallCmd(env *Env) *cobra.Command {
	return &cobra.Command{
		...
		Long: "`dev2 completion zsh` prints a script; this writes it somewhere\n" +
...
func targetFor(shell, home string) (completionTarget, error) {
	...
	case "zsh":
		return completionTarget{
			Path: filepath.Join(home, ".zsh", "completions", "_dev2"),
			Note: "If completions do not appear, add this to ~/.zshrc before compinit:\n" +
				"  fpath=(~/.zsh/completions $fpath)",
		}, nil
	case "bash":
		return completionTarget{
			Path: filepath.Join(home, ".local", "share", "bash-completion", "completions", "dev2"),
			Note: "Requires the bash-completion package; start a new shell to pick it up.",
		}, nil
	case "fish":
		return completionTarget{
			Path: filepath.Join(home, ".config", "fish", "completions", "dev2.fish"),
			Note: "fish loads this automatically in a new shell.",
		}, nil
	default:
		return completionTarget{}, fmt.Errorf(
			"unsupported shell %q; dev2 completion install accepts bash, zsh or fish", shell)
	}
}
