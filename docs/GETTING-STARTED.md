# Quickstart: adopting dev in a repository you already have

[../README.md](../README.md) covers what the tool is. This is the other thing:
...
```sh
cd ~/code/my-project
dev status      # what it detected, before anything runs
dev run -c 'npm test'
```
...
`dev build` says more, and says where the Dockerfile came from:
...
```sh
dev run --egress-prompt ask -c 'npm ci'
dev console -- npm ci                 # same, in the full-screen view
```
...
```sh
dev tools search ripgrep     # what the image's package index has
dev tools add --shared ripgrep jq
```
...
### Option A — run dev per subproject (recommended)

`dev` is directory-scoped. Every subdirectory is its own project with its
own image, its own grants and its own history:

```sh
cd backend  && dev status     # python 3.14 (from pyproject.toml)
cd ../frontend && dev status  # node
```
...
```sh
dev tools add --shared python3 python3-pip
```
...
`dev build` prefers, in order: the project's `Dockerfile`, then its
...
```sh
dev pin       # resolve base images to digests, recorded per project
dev scan      # vulnerabilities in the image you actually run
dev update    # move base images and packages forward, and report what moved
```
...
```sh
dev devcontainer     # writes .devcontainer/, describing the same image
```

They get the same base, the same tools, the same unprivileged uid. They do
**not** get the egress filtering — that lives in a sidecar dev starts, and
...
names neither the culprit nor the fix. dev names both:

```
dev: port 8000 is already published by dev-other-proxy (other-project).
  Free it:  dev clean --all
```

---

## What dev will not do for you
