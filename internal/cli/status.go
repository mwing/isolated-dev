...
func showStatus(ctx context.Context, env *Env, all bool) error {
	...
	running, err := eng.List(ctx, "dev.role")
	if err != nil {
		return err
	}
	...
	for _, c := range running {
		mine := c.Label("dev.project") == p.Name ||
		...
		fmt.Fprintf(env.Stdout, "  %-28s %-10s %-22s %s\n",
			c.Names, c.Label("dev.role"), truncate(c.Image, 22), c.Status)
	}
	if shown == 0 {
	...
	for _, c := range running {
		if c.Label("dev.role") != "egress-sidecar" || !strings.HasPrefix(c.State, "running") {
			continue
		}
...
func cleanAll(ctx context.Context, env *Env) error {
	...
	containers, err := eng.List(ctx, "dev.role")
	if err != nil {
		return err
	}
...
func clean(ctx context.Context, env *Env, images bool) error {
	...
	containers, err := eng.List(ctx, "dev.project="+p.Name)
	if err != nil {
		return err
	}
	sidecar := "dev-" + p.Name + "-proxy"
	...
	nets, err := eng.Networks(ctx, "dev-"+p.Name+"-")
	if err != nil {
		return err
	}
	for _, n := range nets {
	...
	// the user would have to redo, and `dev agent logout` exists to
