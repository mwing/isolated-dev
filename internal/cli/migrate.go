...
func migrate(_ context.Context, env *Env, write bool) error {
	...
	// The name moved, so say where the old tool went. Someone reading this
	// is mid-transition by definition, and "my muscle memory broke and I
	// cannot find the old one" is the first thing that happens.
	fmt.Fprintf(env.Stdout, "\nThe bash tool is now `dev1`, installed from v1/install.sh.\n")
	fmt.Fprintf(env.Stdout, "It still reads ~/.dev-envs, so both see the same config and "+
		"language plugins.\n")
	if _, err := os.Stat(filepath.Join(os.Getenv("HOME"), ".local", "bin", "dev1")); err != nil {
		fmt.Fprintf(env.Stdout, "  (not installed here — run v1/install.sh if you want it back)\n")
	}

	if !write {
		if len(dead) > 0 {
			fmt.Fprintf(env.Stdout, "\nRun `dev migrate --write` to strip the %d dead key(s) "+
