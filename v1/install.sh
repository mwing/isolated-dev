...
uninstall_files() {
    ...
    rm -f "$BIN_DIR/devenv" "$BIN_DIR/dev1"
...
backup_existing "$BIN_DIR/dev1"
# Clean up any old devenv script
rm -f "$BIN_DIR/devenv"

# Installed as dev1: the Go rewrite owns `dev`. Same script, different
# name, so an existing workflow keeps working while nothing shadows the
# tool that replaced it.
cp "$SRC_DIR/scripts/dev" "$BIN_DIR/dev1" || error_exit "Failed to copy dev"
chmod +x "$BIN_DIR/dev1" || error_exit "Failed to make dev1 executable"
...
verify_installation() {
    ...
    [[ -x "$BIN_DIR/dev1" ]] || error_exit "dev1 is not executable"
